...
| 36 | The front pocket vanished a moment after a rig loaded | the pocket is built inside the bar roll's group, so the aero meter listed its meshes under both bags; on the way in the second entry saved the first's id material as the "original", and on the way out put it back, with the bag-only layer. The tunnel always had this; the watts chip, which measures on load, made it show on every rig with a pocket | each mesh belongs to the nearest bag above it, once (`src/aero/measure.js`). Red-pixel test: 84,084 before the measurement, 5 after; fixed, 84,054 after. `aero-check` 28/28 |
| 37 | The wind tunnel shot differed about one run in five after the pocket fix | two timing paths: a kit change landing after the tunnel opened re-seeded the smoke after its fixed burst, and at a few frames a second the 1.5 s crossfade could outlast the shot's wait, so the rider's opacity and the floor labels' draw order varied | under `?still` the smoke is always its seed plus 120 fixed steps and always re-seeds on rebuild; the shot tool waits for `app.aero.transitioning`. S21 10 of 10 exact |

...
const SCREENS = [
  ...
  ['S04', 'empty', `await __builder(); await __bags('first-overnighter'); app.packUI.setMode('gear'); await __w(400); app.ui.setSelected('seatpack'); await __w(1500);`],
  ['S04', 'full', `await __mine(); await __w(500); app.ui.setSelected('seatpack'); await __w(2500);`],
  ['S05', 'full', `await __builder(); await __bags('first-overnighter'); app.packUI.setMode('gear'); await app.pack.gearReady; const it = app.pack.add({ name: 'Tent poles', g: 400, a: 'pole_bundle', d: [55, 5, 5] }, 'toptube'); await __w(300); app.ui.setSelected('toptube'); await __w(2000);`],

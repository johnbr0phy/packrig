 ...
 *
 *   node tools/scratch/_mobile.mjs <url> [w] [h]
 */
...
// The start screen's first row — `Build a rig`, or `My rigs` once there is one.
// It was `.home-btn.is-primary`, a v1 selector that has not existed since the
// menu was rebuilt, so this line quietly clicked nothing and every check below
// ran against the MENU rather than against the builder it is written for.
await p.evaluate(() => document.querySelector('.pr-item')?.click());
await w(900);
// Straight into the mount list now, so close it to get at the rig panel.
// `.sheet-close` exactly: `.sheet-head button` also matches the BACK arrow,
// which sits first in the header and does nothing on the sheet's first level —
// so the sheet stayed open and every check below ran against a panel that was
// `visibility: hidden` underneath it.
...
const audit = await p.evaluate(() => {
  ...
  return {
    ...
    // Save lives in its own bottom-right dock now, and the Menu button is gone:
    // the wordmark is the way up, on a phone as everywhere else.
    save: onScreen('.save-dock'), acct: onScreen('.acct-btn'), up: onScreen('.wordmark'),
    authOn: !!window.app?.auth?.enabled,
...
ok(audit.save === true, 'the save dock is fully on screen');
// Only when there IS an account to reach: a build with `src/config.js` blank
// hides the control on purpose, and asserting on it made every local run fail
// for a feature that is switched off.
if (audit.authOn) ok(audit.acct === true, 'the account button is fully on screen');
else console.log('  --   accounts are switched off in this build; skipping');
ok(audit.up === true, 'the wordmark — the way back to the menu — is fully on screen');

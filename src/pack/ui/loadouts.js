...
export function initLoadouts(app, hooks) {
  ...
  function open() {
    app.openSheet?.({ kind: 'detail', title: 'Loadouts', render: (body, h) => paint(body, h) });
  }
  ...
  function paint(body, h) {
    ...
    for (const lo of P.loadouts()) {
      ...
      more.append(
        btn('pkg-mini', 'Rename', () => rename(row, lo, body, h), { label: `Rename ${lo.name}` }),
        btn('pkg-mini', 'Copy', () => { P.duplicate(lo.id); paint(body, h); }, { label: `Duplicate ${lo.name}` }),
        btn('pkg-mini is-bad', 'Delete', () => {
          const at = P.loadouts().indexOf(lo);
          const gone = P.deleteLoadout(lo.id);
          paint(body, h);
          hooks.notify?.(`Deleted ${gone?.name || 'loadout'}`, () => { P.restoreLoadout(gone, at); paint(body, h); });
    ...
    acts.append(
      btn('pkg-btn is-primary', 'New loadout', () => { const lo = P.newLoadout('New loadout', { fromCurrent: true }); paint(body, h); const r = body.querySelector('.pkg-lo.on'); if (r) rename(r, lo, body, h); }),
      btn('pkg-btn', 'Compare', () => openCompare()),
    );
    wrap.append(acts);
    const io = el('div', 'pkg-row-actions');
    io.append(
      btn('pkg-btn', 'Paste a spreadsheet', () => openImport()),
      btn('pkg-btn', 'Copy as spreadsheet', async (e) => {
      ...
      btn('pkg-btn', 'Download .tsv', () => download(`${(P.active()?.name || 'loadout').replace(/[^\w-]+/g, '-')}.tsv`, P.exportText())),
    );
    wrap.append(io);
    body.append(wrap);
  }
  ...
  function rename(row, lo, body, h) {
    ...
    const inp = el('input', 'pkg-in');
    inp.value = lo.name || '';
    inp.setAttribute('aria-label', 'Loadout name');
    inp.maxLength = 60;
    f.append(inp, Object.assign(el('button', 'pkg-btn', 'Save'), { type: 'submit' }));
  ...
  function paintCompare(body, aId, bId) {
    ...
    wrap.append(el('div', 'bs-label', moved.length ? `What moved · ${moved.length}` : 'Nothing moved'));
  ...
  function paintImport(body, h) {
    ...
    const name = el('input', 'pkg-in');
    name.placeholder = 'Name this loadout';
    name.setAttribute('aria-label', 'Loadout name');
    const ta = el('textarea', 'pkg-ta');
    ta.rows = 10;
    ...
    const go = el('button', 'pkg-btn is-primary', 'Import');
    go.type = 'button';
    go.disabled = true;
    let last = null;
    ta.oninput = async () => {
    ...
    go.onclick = async () => {
      const res = await P.importText(ta.value, { name: name.value.trim() || 'Imported loadout' });
      h?.close();
